...
df = pd.DataFrame(columns = ["program","fp64_NAN", "fp64_INF", "fp64_SUB","fp32_NAN", "fp32_INF", "fp32_SUB","kernel","FP instructions","check_time","ori_time","slowdown"])
...
def create_table(dirs):
    for dir in dirs:
        ...
        # print(run_command([comd]))
            # for x in glob.glob("./*"):
            #     print(x)
            with open("stdout.soap.perf.txt") as f:
                exist = 0
            ...
            if(exist != 0):
                with open("stderr.soap.perf.txt") as time_f:
                    infos = time_f.readlines()
                    t_s = time_info(infos)
                    e.append(t_s)
                with open("stderr_ori.soap.perf.txt") as timeori_f:
...
def entry(start_dir):
    dir_list = []
    for f in glob.iglob(start_dir+'/**/stdout.soap.perf.txt',recursive=True):
        d = '/'.join(f.split('/')[:-1])
        dir_list.append(d)
